...
type umTradeDownloadLinkServiceTestSuite struct {
	baseTestSuite
}

func TestUMTradeDownloadLinkService(t *testing.T) {
	suite.Run(t, new(umTradeDownloadLinkServiceTestSuite))
}

func (s *umTradeDownloadLinkServiceTestSuite) TestGetUMTradeDownloadLink() {
	data := []byte(`{
	...
	res, err := s.client.NewGetUMTradeDownloadLinkService().
