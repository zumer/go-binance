...
type umOrderHistoryServiceTestSuite struct {
	baseTestSuite
}

func TestUMOrderHistoryService(t *testing.T) {
	suite.Run(t, new(umOrderHistoryServiceTestSuite))
}

func (s *umOrderHistoryServiceTestSuite) TestGetUMOrderHistoryDownloadID() {
	data := []byte(`{
	...
	res, err := s.client.NewGetUMOrderHistoryDownloadIDService().
		StartTime(startTime).
		EndTime(endTime).
