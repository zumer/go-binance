...
type umTransactionHistoryServiceTestSuite struct {
	baseTestSuite
}

func TestUMTransactionHistoryService(t *testing.T) {
	suite.Run(t, new(umTransactionHistoryServiceTestSuite))
}

func (s *umTransactionHistoryServiceTestSuite) TestGetUMTransactionHistoryDownloadID() {
	data := []byte(`{
	...
	res, err := s.client.NewGetUMTransactionHistoryDownloadIDService().
		StartTime(startTime).
		EndTime(endTime).
